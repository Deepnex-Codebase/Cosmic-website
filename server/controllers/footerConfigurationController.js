...
const uploadFooterLogo = async (req, res) => {
  try {
    ...
    // If there's an existing logo and it's not the default, delete it
    if (footerConfig.companyInfo.logo && footerConfig.companyInfo.logo !== 'logo.png') {
      const oldLogoPath = path.join(__dirname, '../uploads/footer', footerConfig.companyInfo.logo.split('/').pop());
    ...
    res.status(500).json({ message: 'Error uploading footer logo', error: error.message });
  }
};
