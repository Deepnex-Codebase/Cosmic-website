...
exports.updateService = async (req, res) => {
  try {
    ...
    // Handle image upload
    if (req.file) {
      console.log('Image file received in controller:', req.file.filename);
      // Get existing service to delete old image
      const existingService = await Service.findById(id);
      if (existingService && existingService.image && (existingService.image.startsWith('/uploads/') || existingService.image.startsWith('uploads/'))) {
        const imagePath = existingService.image.startsWith('/') ? existingService.image : '/' + existingService.image;
      ...
      console.log('New image path set:', updateData.image);
    } else {
      console.log('No image file received in update request');
    }
...
exports.deleteService = async (req, res) => {
  try {
    ...
    if (service.image && (service.image.startsWith('/uploads/') || service.image.startsWith('uploads/'))) {
      const imagePath = service.image.startsWith('/') ? service.image : '/' + service.image;
      const oldImagePath = path.join(__dirname, '..', imagePath);
      if (fs.existsSync(oldImagePath)) {
        fs.unlinkSync(oldImagePath);
      }
...
// Upload middleware
exports.uploadServiceImage = serviceUpload.single('image');
