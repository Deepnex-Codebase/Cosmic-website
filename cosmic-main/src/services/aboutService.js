...
export const formatImageUrl = (imageUrl) => {
  ...
  // If it's already a complete URL, check if it's localhost or api URL and replace with production URL
  if (imageUrl.startsWith('http')) {
    return imageUrl
      .replace('http://localhost:5174', 'https://api.cosmicpowertech.com')
      .replace('http://api.cosmicpowertech.com', 'https://api.cosmicpowertech.com');
  }
  
  // If it's a relative path, append the API base URL
  return `https://api.cosmicpowertech.com${imageUrl.startsWith('/') ? '' : '/'}${imageUrl}`;
};
...
export const uploadExpertiseImage = async (imageFile) => {
  try {
    // Check file size before uploading (limit to 40MB)
    if (imageFile.size > 40 * 1024 * 1024) {
      throw new Error('Image size exceeds 40MB limit. Please compress the image or choose a smaller one.');
    }
    ...
    if (error.response && error.response.status === 413) {
      throw new Error('Image size is too large for the server. Please use an image smaller than 40MB.');
...
export const uploadHeroVideo = async (videoFile) => {
  try {
    // Check file size before uploading (limit to 200MB)
    if (videoFile.size > 200 * 1024 * 1024) {
      throw new Error('Video size exceeds 200MB limit. Please compress the video or choose a smaller one.');
    }
    ...
    // Provide more specific error messages
    if (error.response && error.response.status === 413) {
      throw new Error('Video size is too large for the server. Please use a video smaller than 200MB.');
    } else if (error.code === 'ERR_NETWORK') {
      throw new Error('Network error. Please check your connection and try again.');
    }
    throw error;
